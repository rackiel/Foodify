...
<div class="container-fluid py-4">
        ...
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted mb-0">Real-time analytics of community engagement and food sharing activities - Administrative Dashboard</p>
                </div>
                <div class="btn-group">
    ...
    <div class="row mb-4">
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0"><i class="bi bi-shield-check"></i> Platform Health Status</h5>
                </div>
                <div class="card-body">
                    <div class="row text-center">
                        <div class="col-4">
                        ...
                        <div class="col-4">
                            <div class="health-stat">
                                ...
                                <h4 class="text-info"><?= number_format($stats['feedback']['avg_rating'] ?? 0, 1) ?></h4>
                        ...
                        <div class="col-4">
                            <div class="health-stat">
                                ...
                                <h4 class="text-primary"><?= 
                                    count($stats['announcement_trend']) > 1 ? 
                                    ($stats['announcement_trend'][count($stats['announcement_trend'])-1]['count'] - $stats['announcement_trend'][0]['count']) : 0 
                                ?></h4>
                                <small>Growth</small>
                            </div>
                        </div>
                    ...
                    <div class="alert alert-info">
                        ...
                        <?php 
                        $health_score = 0;
                        if ($stats['active_users'] > 0) $health_score += 25;
                        if ($stats['feedback']['avg_rating'] >= 4) $health_score += 25;
                        if ($stats['announcements']['total_announcements'] > 0) $health_score += 25;
                        if ($stats['food_donations']['total_donations'] > 0) $health_score += 25;
                        
                        if ($health_score >= 75) {
                            echo '<span class="text-success">Excellent</span> - Platform is performing very well';
                        } elseif ($health_score >= 50) {
                            echo '<span class="text-warning">Good</span> - Platform is performing well';
                        } else {
                            echo '<span class="text-danger">Needs Attention</span> - Consider boosting engagement';
                        }
                        ?>
                    </div>
                </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-6 mb-3">
                            <a href="donation-management.php" class="btn btn-outline-warning w-100">
                                <i class="bi bi-basket"></i><br>
                                <small>Manage Donations</small>
                            </a>
                        </div>
                        <div class="col-6 mb-3">
                            <a href="user-reports.php" class="btn btn-outline-info w-100">
                                <i class="bi bi-people"></i><br>
                                <small>User Reports</small>
                            </a>
                        </div>
                        <div class="col-6 mb-3">
                            <a href="community-feedback.php" class="btn btn-outline-success w-100">
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0"><i class="bi bi-exclamation-triangle"></i> System Alerts & Recommendations</h5>
                </div>
                <div class="card-body">
                    <div id="systemAlerts">
                        <?php
                        ...
                        if (($stats['announcements']['total_likes'] ?? 0) + ($stats['announcements']['total_comments'] ?? 0) < 10) {
                            $alerts[] = [
                                'type' => 'warning',
                                'icon' => 'heart',
                                'title' => 'Low Engagement',
                                'message' => 'Consider creating more interactive content to boost community engagement.'
                            ];
                        }
                        
                        // Check for pending requests
                        if (($stats['reservations']['pending_requests'] ?? 0) > 5) {
                            $alerts[] = [
                                'type' => 'info',
                                'icon' => 'clock',
                                'title' => 'Pending Requests',
                                'message' => $stats['reservations']['pending_requests'] . ' food requests are awaiting approval.'
                            ];
                        }
                        
                        // Check for low ratings
                        if (($stats['feedback']['avg_rating'] ?? 0) < 3 && ($stats['feedback']['total_feedback'] ?? 0) > 0) {
                            $alerts[] = [
                                'type' => 'danger',
                                'icon' => 'star',
                                'title' => 'Low Ratings',
                                'message' => 'Community satisfaction is below average. Review recent feedback.'
                            ];
                        }
                        
                        // Check for inactive users
                        if (($stats['active_users'] ?? 0) < 5) {
                            $alerts[] = [
                                'type' => 'warning',
                                'icon' => 'people',
                                'title' => 'Low User Activity',
                                'message' => 'Consider promoting the platform to increase user participation.'
                            ];
                        }
                        ...
                        if (empty($alerts)) {
                            $alerts[] = [
                                ...
                                'message' => 'Platform is running smoothly with good engagement levels.'
                            ];
                        }
...
// Enhanced Functions for Team Officers
function exportToExcel() {
    // Create a simple CSV export
    const data = [
        ['Metric', 'Value'],
        ['Total Announcements', <?= $stats['announcements']['total_announcements'] ?? 0 ?>],
        ['Total Likes', <?= $stats['announcements']['total_likes'] ?? 0 ?>],
        ['Total Comments', <?= $stats['announcements']['total_comments'] ?? 0 ?>],
        ['Total Shares', <?= $stats['announcements']['total_shares'] ?? 0 ?>],
        ['Total Saves', <?= $stats['announcements']['total_saves'] ?? 0 ?>],
        ['Food Donations', <?= $stats['food_donations']['total_donations'] ?? 0 ?>],
        ['Available Donations', <?= $stats['food_donations']['available_donations'] ?? 0 ?>],
        ['Active Users', <?= $stats['active_users'] ?? 0 ?>],
        ['Average Rating', <?= number_format($stats['feedback']['avg_rating'] ?? 0, 2) ?>],
        ['Total Views', <?= $stats['food_donations']['total_views'] ?? 0 ?>]
    ];
    ...
    a.href = url;
    a.download = 'community_impact_' + new Date().toISOString().split('T')[0] + '.csv';
